...
def diagnostic_differences(ds1, ds2, T = 10):
    ...
    Documentation
    -------------
    Compute distributional and spectral differences between two xarray.Dataset objects 
    representing ensembles of pyqg simulations. Variable shape should be (time, lev, y, x) 
    with an optional leading "run" dimension.
    ...
    def spectral_rmse(spec1, spec2):
        ...
        # Size needed for dimension correction
        spec1_size = len(spec1.data.shape)
        spec2_size = len(spec2.data.shape)
        ...
        kr1, ispec1 = pyqg.diagnostic_tools.calc_ispec(m1, spec1_data if spec1_size == 3 else spec1.data)
        kr2, ispec2 = pyqg.diagnostic_tools.calc_ispec(m2, spec2_data if spec2_size == 3 else spec2.data)
...
def diagnostic_similarities(model, target, baseline, **kw):
    ...
    Documentation
    -------------
    Like `diagnostic_differences`, but returning a dictionary of similarity scores between negative 
    infinity and 1 which quantify how much closer the diagnostics of a given `model` are to a `target` 
    with respect to a `baseline`. Scores approach 1 when the distance between the model and the target 
    is small compared to the baseline and are negative when that distance is greater.
    """
